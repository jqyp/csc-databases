...
WITH Records AS (
  SELECT P.id AS planet_id,
      format('{"commander": "%s", "message": "%s"}', C.name, 'Поехали!')::JSONB AS log_record
  FROM Flight F JOIN Planet P ON F.planet_id=P.id
  JOIN Commander C ON C.id = F.commander_id
  WHERE C.name = 'Гагарин'
  AND random()>0.5
)
UPDATE Planet AS P SET wiki_jsonb =
 jsonb_set(wiki_jsonb, '{log}', wiki_jsonb->'log' || log_record)
...
INSERT INTO Vertex(id, value, parent_id)
VALUES (1, 'm', NULL), (2, 'a', 1), (3, 'r', 2), (4, 's', 3), (5, 'o', 1), (6, 'o', 5), (7, 'n', 6);
